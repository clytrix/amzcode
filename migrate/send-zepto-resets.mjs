...
async function main() {
  ...
  for (const user of users) {
    try {
      ...
      // Store the token in Supabase (create a simple password_resets table if not exists)
      ...
      if (upsertError) {
        // If table doesn't exist, just log and continue - we'll handle reset via client-side
        console.log(`⚠️ ${user.email}: Could not store token - ${upsertError.message}`);
      }
      
      // Build reset URL
      const resetUrl = `${SITE_URL}/reset-password?token=${token}&email=${encodeURIComponent(user.email)}`;
